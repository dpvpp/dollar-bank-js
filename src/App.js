...
function App() {
  ...
  const[accounts, setAccounts] = useState(new Map([
    ['1234', { pin: '1234', balance: (1234).toFixed(2), transactions: ["Initial Deposit: $" + (1234).toFixed(2) + " " + new Date()]}],
    ['1111', { pin: '1111', balance: (1111).toFixed(2), transactions: ["Initial Deposit: $" + (1111).toFixed(2) + " " + new Date()]}],
    ['2222', { pin: '2222', balance: (2341234).toFixed(2), transactions: ["Initial Deposit: $" + (1232).toFixed(2) + " " + new Date()]}],
    ['3333', { pin: '3333', balance: (23423).toFixed(2), transactions: ["Initial Deposit: $" + (1232).toFixed(2) + " " + new Date()]}],
    ['4444', { pin: '4444', balance: (1231).toFixed(2), transactions: ["Initial Deposit: $" + (1232).toFixed(2) + " " + new Date()]}],
    ['5555', { pin: '5555', balance: (13412).toFixed(2), transactions: ["Initial Deposit: $" + (1232).toFixed(2) + " " + new Date()]}]
  ]));
  ...
  const submitAccount = (amount) => {
    if(accounts.get(pin) != null) {
      ...
      setAccount({pin: pin, balance: parseFloat(amount).toFixed(2), transactions: ["Initial Deposit: $" + parseFloat(amount).toFixed(2) + " " + new Date()]});
      setAccounts(new Map(accounts.set(pin, {pin: pin, balance: parseFloat(amount).toFixed(2), transactions: ["Initial Deposit: $" + parseFloat(amount).toFixed(2) + " " + new Date()]})));
      setMenu(2);
      setError(0);
    }
  }
  ...
  const submitWithdrawal = (amount) => {
    if(parseFloat(account.balance) < parseFloat(amount)) {
      ...
      update.transactions.push("Withdrawal: $" + parseFloat(amount).toFixed(2) + " " + new Date())
  ...
  const submitDeposit = (amount) => {
    ...
    let update = {pin: account.pin, balance: account.balance, transactions: account.transactions};
    update.balance = (parseFloat(update.balance) + parseFloat(amount)).toFixed(2);
    update.transactions.push("Deposit: $" + parseFloat(amount).toFixed(2) + " " + new Date())
